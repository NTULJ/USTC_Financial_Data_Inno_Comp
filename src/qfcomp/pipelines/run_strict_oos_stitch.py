...
from qfcomp.factors.testing import test_all_factors
...
def main() -> None:
    ...
    no_bayes_first_n_outer = int(args.no_bayes_first_n_outer)
    outer_test_months = _resolve_period_months(
        years=int(args.outer_test_years),
        months=int(args.outer_test_months),
        name="outer-test",
    )
    outer_step_months = _resolve_period_months(
        years=int(args.outer_step_years),
        months=int(args.outer_step_months),
        name="outer-step",
    )
    inner_train_months = _resolve_period_months(
    ...
    if reuse_run_dir is not None:
        ...
        print("=" * 70)
        print("Step 1: fresh 计算信号（固定因子+固定合成）")
    ...
    for outer in outer_folds:
        ...
        inner_folds = _build_inner_folds_with_fallback(
            ...
            step_months=int(inner_step_months),
        ...
        # 用本段独立参数，仅运行本段 OOS
        raw_all = build_optimized_schedule(
            composite.loc[:outer_end],
            close_matrix.loc[:outer_end],
            optimizer=OPTIMIZER_METHOD,
