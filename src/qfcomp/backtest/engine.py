...
def _ensure_writable_plot_cache_dirs() -> None:
    ...
    mpl_config_dir = Path(os.environ.get("MPLCONFIGDIR", "/tmp/qfcomp_mplconfig"))
    font_cache_dir = Path(os.environ.get("XDG_CACHE_HOME", "/tmp/qfcomp_cache"))
    try:
...
def extract_nav(result,
    ...
    for col in result.prices.columns:
        full_nav = result.prices[col]
        # 截取回测起始日之后
        nav = full_nav.loc[full_nav.index >= start_date].copy()
        if len(nav) == 0:
            continue
        # 归一化到 1.0
        nav = nav / nav.iloc[0]
