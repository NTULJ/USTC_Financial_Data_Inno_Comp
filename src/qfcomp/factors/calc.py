...
============
职责：
  1. 复用 qfcomp.factors.library 的因子计算逻辑
...
import numpy as np
import pandas as pd
from qfcomp.factors.library import compute_all_factors, load_product_pool

from qfcomp.config import PANEL_FACTORS, MACRO_FACTORS, AUX_FACTORS, DATA_DIR
...
def compute_factors(aligned: pd.DataFrame) -> tuple:
    ...
    panel_wide : dict[str, pd.DataFrame]
        {因子名: DataFrame(index=date, columns=sec)} 的字典
