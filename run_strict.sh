...
mkdir -p logs
LOG_FILE="logs/strict_oos_$(date +%Y%m%d_%H%M%S).log"

nohup zsh -lc '
PYTHONPATH=src python src/qfcomp/pipelines/run_strict_oos_stitch.py \
  --n-trials 180 \
  --n-startup-trials 36 \
  --seed 42 \
